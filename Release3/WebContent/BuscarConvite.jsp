		...
		<c:otherwise>
			<div id="ordenar_buscar">
				<form action="FiltrarConvitesServlet">
					...
					</select> <input type="text" name="filtro" id="filtro"
						placeholder="Busca tu convite">
					...
					<select name="selectOrden">
						<c:choose>
							<c:when test="${selectedOrder == 2}">
								...
								<option value="precio_asc" selected>Ordenar por precio
									(ascendente)</option>
								<option value="precio_desc">Ordenar por precio
									(descendente)</option>
							</c:when>
							<c:when test="${selectedOrder == 3}">
								<option value="fecha">Ordenar por fecha</option>
								<option value="precio_asc">Ordenar por precio
									(ascendente)</option>
								<option value="precio_desc" selected>Ordenar por precio
									(descendente)</option>
							</c:when>
							<c:otherwise>
